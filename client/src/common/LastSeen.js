...
function LastSeen(props) {
    ...
    useEffect(() => {
        ...
        if(lastSeen){
            const expiry = localStorage.getItem(LAST_SEEN_EXPIRY_LOCALSTORAGE_KEY);
        ...
        fetchSeen().then((json) => {
            if (json.error) {
                ...
                const expires = Math.floor(Date.now() / 1000) + GITHUB_RATE_LIMIT_SECONDS
                localStorage.setItem(LAST_SEEN_LOCALSTORAGE_KEY, JSON.stringify(json));
                localStorage.setItem(LAST_SEEN_EXPIRY_LOCALSTORAGE_KEY, expires)
                setSeen(json);
            }
        });
    }, []);
    ...
    return(
        <React.Fragment>
            <Typography variant={"caption"}  color="textSecondary">
                Last posted: {seen.lastPost ? createLastPostedElement(seen.lastPost) : <CircularProgress size={10}/>}
            </Typography>
            <br />
            <Typography variant={"caption"} color="textSecondary">
                Code updated: {seen.lastCommit ? createCodeUpdatedElement(seen.lastCommit) : <CircularProgress size={10}/>}
            </Typography>
        </React.Fragment>
    )
}
...
function createCodeUpdatedElement(lastCommit){
    return(
        <Tooltip title={lastCommit.message}>
            <a target={"_blank"} style={styles.commitUrl} href={lastCommit.url}>{lastCommit.date}</a>
        </Tooltip>
    )
}
